  ...
  namespace homekit
  {
    ...
    private:
      ...
      static int light_write(hap_write_data_t write_data[], int count, void* serv_priv, void* write_priv) {
        ...
        ESP_LOGI("light_write", "Write called for Accessory %s", (char*)serv_priv);
      ...
      void on_light_update(light::LightState* obj) {
        ESP_LOGI("on_light_update", "%s RED: %.2f, GREEN: %.2f, BLUE: %.2f", obj->get_name().c_str(), obj->current_values.get_red(), obj->current_values.get_green(), obj->current_values.get_blue());
        ESP_LOGI("on_light_update", "%s state: %d level: %d", obj->get_name().c_str(), (int)(obj->current_values.get_state() * 100), (int)(obj->current_values.get_brightness() * 100));
      ...
      void setup(std::vector<light::LightState*> &excluded) {
        ...
        for (auto entity : App.get_lights()) {
          ...
          std::string accessory_name = entity->get_name();
            ...
            if (entity->current_values.get_color_mode() & light::ColorCapability::RGB) {
                ...
                hap_serv_add_char(service, hap_char_saturation_create(saturation));
            }
            if (entity->current_values.get_color_mode() & light::ColorCapability::COLOR_TEMPERATURE) {
                hap_serv_add_char(service, hap_char_color_temperature_create(entity->current_values.get_color_temperature_kelvin()));
            }
